import { Landmark, Quote, Users, Sparkles } from 'lucide-react';
...
export default function AboutAndReviews({ currentLang }: AboutAndReviewsProps) {
  ...
  return (
    <section id="about" className="relative py-24 bg-white overflow-hidden">
      ...
      <div className="absolute -top-32 -left-24 w-[480px] h-[480px] rounded-full opacity-30 blur-3xl pointer-events-none"
           style={{ background: 'radial-gradient(closest-side, rgba(244,63,94,0.25), transparent 70%)', animation: 'blob-morph 22s ease-in-out infinite' }} />
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Reveal as="div" className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center mb-24">
          ...
          <div className="lg:col-span-6 space-y-6 text-left">
            <div className="inline-flex items-center space-x-1 bg-rose-50 text-rose-700 px-3.5 py-1 rounded-full text-xs font-bold tracking-wider mb-2 uppercase">
              <Landmark className="w-3.5 h-3.5" />
              <span>{t.navAbout}</span>
            </div>
            
            <h3 className="font-sans text-3xl font-extrabold text-slate-900 tracking-tighter leading-tight">
            ...
            <div className="grid grid-cols-2 gap-6 pt-4 border-t border-slate-100">
              <div className="space-y-1">
                <span className="font-mono text-3xl font-black text-rose-650 block">100,000+</span>
                <span className="text-xs font-bold text-slate-450 uppercase tracking-wider">
                  {currentLang === 'en' ? 'Active Users Serviced' : '服务调顺气血家庭'}
                </span>
              </div>
              <div className="space-y-1">
                <span className="font-mono text-3xl font-black text-rose-600 block">20+</span>
                <span className="text-xs font-bold text-slate-450 uppercase tracking-wider">
          ...
          <div className="lg:col-span-6 relative flex justify-center w-full">
            ...
            <div className="bg-[#FAFAFB] rounded-[2.5rem] p-8 sm:p-10 border-2 border-slate-150 shadow-xl text-left w-full max-w-lg relative overflow-hidden">
              ...
              <div className="space-y-6">
                <div className="flex items-center space-x-2.5">
                  ...
                  <div>
                    <span className="text-[10px] font-mono font-black text-rose-600 tracking-wider block">CORE RESEARCH & CAPABILITIES</span>
                    <h4 className="text-base font-extrabold text-slate-900 font-sans">{currentLang === 'en' ? 'Biomedical Competence' : '中科医用 · 科技与研发核心'}</h4>
                  </div>
                </div>

                <div className="space-y-4 text-xs font-bold text-slate-600 leading-relaxed">
                  <div className="p-3.5 bg-white border border-slate-200/50 rounded-2xl space-y-1 hover:border-rose-100 transition-colors">
                    <span className="font-sans text-rose-650 text-[11px] block">01 / {currentLang === 'en' ? 'Active Biopolymers' : '活性医用高分子材料研发'}</span>
                    <p className="text-[11px] leading-relaxed text-slate-500 font-semibold font-sans">
                      {currentLang === 'en' 
                  ...
                  <div className="p-3.5 bg-white border border-slate-200/50 rounded-2xl space-y-1 hover:border-rose-100 transition-colors">
                    <span className="font-sans text-rose-650 text-[11px] block">02 / {currentLang === 'en' ? 'Endogenous Nitric Oxide Conversion' : '内源一氧化氮信号通路日常化'}</span>
                    <p className="text-[11px] leading-relaxed text-slate-500 font-semibold font-sans">
                      {currentLang === 'en' 
                  ...
                  <div className="p-3.5 bg-white border border-slate-200/50 rounded-2xl space-y-1 hover:border-rose-150 transition-colors">
                    <span className="font-sans text-rose-650 text-[11px] block">03 / {currentLang === 'en' ? 'National Clinical & Quality Standards' : '国家级多中心临床验证'}</span>
                    <p className="text-[11px] leading-relaxed text-slate-500 font-semibold font-sans">
                      {currentLang === 'en' 
                ...
                <div className="border-t border-slate-150 pt-3 mt-4 flex gap-3 items-center">
                  ...
                  <div>
                    ...
                    <span className="text-[11px] font-black text-slate-800 leading-none">北京中科医用材料有限公司 / 高科技材料成果转化中心</span>
                  </div>
                </div>
        ...
        <div className="space-y-10">
          <div className="text-center max-w-2xl mx-auto">
            <h3 className="font-sans text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tighter leading-tight">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {simulatedReviews.map((rev, index) => (
              <motion.div
                ...
                whileHover={{ y: -4 }}
                className="relative bg-white p-6 sm:p-8 rounded-3xl border-2 border-slate-150 hover:border-rose-300 transition-colors duration-300 flex flex-col justify-between text-left shadow-xl shadow-slate-100/40 hover:shadow-2xl hover:shadow-rose-100/20 overflow-hidden"
              >
                {/* Subtle molecule peeking in the top-right */}
                <div className="absolute -top-3 -right-3 opacity-40">
                  <Molecule size={56} glow="rose" animation="float" rotate={-15} />
                </div>

                <div className="space-y-3.5 relative">
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="text-amber-500 text-sm select-none">{"★".repeat(rev.stars)}</div>
                      <h4 className="font-sans text-base font-extrabold text-slate-900 mt-1">{rev.title}</h4>
                    </div>
                    <span className="text-[10px] font-mono font-bold text-slate-400">{rev.date}</span>
                  </div>

                  <p className="text-slate-600 text-xs sm:text-sm leading-relaxed italic font-semibold">
                    " {rev.content} "
                  </p>
                </div>

                <div className="relative border-t border-slate-150 pt-4 mt-5 flex justify-between items-center text-xs">
                  <span className="font-bold text-slate-800">{rev.author}</span>
                  <span className="font-mono font-bold text-rose-600 bg-rose-50 border border-rose-100/50 py-0.5 px-2.5 rounded-full text-[10px]">
                    {rev.product}
                  </span>
                </div>
              </motion.div>
