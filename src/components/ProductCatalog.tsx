...
import { Tag, ShoppingCart, Eye, X, Award, BadgeCheck } from 'lucide-react';
...
export default function ProductCatalog({ currentLang, currency, onAddToCart, onSelectProduct, shoppingEnabled = true, showPrices = true }: ProductCatalogProps) {
  ...
  const renderProductGraphic = (prod: Product, colorHex: string) => {
    if (prod.images && prod.images.length > 0) {
      return (
        <div className="relative w-full h-56 bg-gradient-to-br from-neutral-50 to-neutral-100 overflow-hidden border-b border-gray-100">
          <img
            src={prod.images[0]}
            ...
            loading="lazy"
          />
          <div className="absolute top-3 right-3 flex items-center gap-1.5 bg-white/80 backdrop-blur-xs py-0.5 px-1.5 rounded-md border border-gray-100">
            <Molecule size={22} glow="rose" animation="pulse" />
            <span className="text-[9px] font-mono font-black text-gray-500">THERABO · NO</span>
          </div>
        </div>
      );
    }
    ...
    return (
      <div className="relative w-full h-56 bg-gradient-to-br from-neutral-50 to-neutral-100 flex items-center justify-center p-6 border-b border-gray-100 group-hover:from-red-50/20 group-hover:to-sky-50/20 transition-colors duration-500 overflow-hidden">
        
        {/* Dynamic Glowing background representing NO wave synthesis propagation */}
        <div className="absolute inset-0 bg-radial-gradient opacity-0 group-hover:opacity-40 transition-opacity duration-700 pointer-events-none" />
        ...
        {prod.id === 'no-sport-knee-support' && (
          <svg className="w-32 h-32 text-gray-700 drop-shadow-lg animate-pulse" viewBox="0 0 100 100" fill="none">
            ...
            <path d="M25 35 Q50 45 75 35" stroke="#EF4444" strokeWidth="2.5" opacity="0.8" />
            <path d="M25 65 Q50 75 75 65" stroke="#14B8A6" strokeWidth="2.5" opacity="0.8" />
            <circle cx="50" cy="50" r="16" fill="none" stroke="#EF4444" strokeWidth="1.5" strokeDasharray="3 3" />
        ...
        {prod.id === 'no-waist-belt-fixed' && (
          <svg className="w-40 h-28 text-gray-700 drop-shadow-lg" viewBox="0 0 120 80" fill="none">
            <rect x="10" y="20" width="100" height="40" rx="12" fill={colorHex} opacity="0.85" stroke="#EF4444" strokeWidth="1.5" />
            ...
            <circle cx="60" cy="40" r="10" fill="none" stroke="#EF4444" strokeWidth="1.5" strokeDasharray="3 3" />
            {brandDots}
          </svg>
        )}
        ...
        {prod.id === 'no-waist-belt-support' && (
          <svg className="w-40 h-28 text-gray-700 drop-shadow-lg" viewBox="0 0 120 80" fill="none">
            ...
            <circle cx="75" cy="40" r="8" fill="none" stroke="#EF4444" strokeWidth="1.5" strokeDasharray="2 2" />
        ...
        {prod.id === 'no-neck-collar' && (
          <svg className="w-28 h-28 text-gray-700 drop-shadow-lg" viewBox="0 0 100 100" fill="none">
            ...
            <circle cx="50" cy="49" r="8" fill="none" stroke="#EF4444" strokeWidth="1.5" strokeDasharray="2 2" />
            {brandDots}
          </svg>
        )}
        ...
        {prod.id === 'no-men-underwear' && (
          <svg className="w-32 h-32 text-gray-700 drop-shadow-lg" viewBox="0 0 100 100" fill="none">
            ...
            <circle cx="50" cy="55" r="10" fill="none" stroke="#EF4444" strokeWidth="2" strokeDasharray="2 2" />
            {brandDots}
          </svg>
        )}
        ...
        {prod.id === 'no-men-sleeve' && (
          <svg className="w-32 h-32 text-gray-700 drop-shadow-lg" viewBox="0 0 100 100" fill="none">
            ...
            <circle cx="50" cy="50" r="14" fill="none" stroke="#EF4444" strokeWidth="1.5" strokeDasharray="3 3" />
            {brandDots}
          </svg>
        )}
        ...
        {prod.id === 'no-women-sleeve' && (
          <svg className="w-32 h-32 text-gray-700 drop-shadow-lg" viewBox="0 0 100 100" fill="none">
            ...
            <circle cx="50" cy="52" r="14" fill="none" stroke="#EF4444" strokeWidth="1.5" strokeDasharray="3 3" />
            {brandDots}
          </svg>
        )}
        ...
        {prod.id === 'therabo-therapy-boots' && (
          <svg className="w-32 h-32 text-gray-700 drop-shadow-lg" viewBox="0 0 100 100" fill="none">
            ...
            <circle cx="37" cy="40" r="10" fill="none" stroke="#EF4444" strokeWidth="1.5" strokeDasharray="2 2" />
        ...
        {prod.id === 'no-breast-patch' && (
          <svg className="w-28 h-28 text-gray-700 drop-shadow-lg" viewBox="0 0 100 100" fill="none">
            ...
            <circle cx="50" cy="50" r="28" fill="none" stroke="#EF4444" strokeWidth="1" strokeDasharray="3 3" />
        ...
        {prod.id === 'no-eye-mask' && (
          <svg className="w-36 h-24 text-gray-700 drop-shadow-lg" viewBox="0 0 100 60" fill="none">
            ...
            <circle cx="30" cy="30" r="6" fill="none" stroke="#EF4444" strokeWidth="1.2" strokeDasharray="2 2" />
            <circle cx="70" cy="30" r="6" fill="none" stroke="#EF4444" strokeWidth="1.2" strokeDasharray="2 2" />
            {brandDots}
          </svg>
        )}
        ...
        {prod.id === 'no-face-mask' && (
          <svg className="w-30 h-30 text-gray-700 drop-shadow-lg" viewBox="0 0 100 100" fill="none">
            ...
            <ellipse cx="50" cy="72" rx="10" ry="4" stroke="#EF4444" strokeWidth="1.5" fill="none" />
            {brandDots}
          </svg>
        )}
        ...
        {prod.id === 'no-gloves' && (
          <svg className="w-32 h-32 text-gray-700 drop-shadow-lg" viewBox="0 0 100 100" fill="none">
            ...
            <circle cx="50" cy="62" r="10" fill="none" stroke="#EF4444" strokeWidth="1.5" strokeDasharray="2 2" />
            {brandDots}
          </svg>
        )}
        ...
        {prod.id === 'therabo-energy-chamber' && (
          <svg className="w-40 h-28 text-gray-700 drop-shadow-lg" viewBox="0 0 120 80" fill="none">
            ...
            <line x1="15" y1="42" x2="105" y2="42" stroke="#EF4444" strokeWidth="1.5" />
    ...
    <section id="products" className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Reveal>
          <div className="text-center max-w-3xl mx-auto mb-16">
            <div className="inline-flex items-center gap-1.5 bg-rose-50 text-rose-700 px-3.5 py-1 rounded-full text-xs font-bold tracking-wider mb-4 uppercase border border-rose-100">
              <Tag className="w-3.5 h-3.5" />
              <span>{t.productTitle}</span>
            </div>
            <h2 className="font-sans text-3xl sm:text-4xl font-extrabold text-slate-900 tracking-tighter leading-tight">
              {t.productTitle}
            </h2>
        ...
        <div className="flex justify-center mb-12">
          <div className="inline-flex flex-wrap justify-center gap-1 bg-slate-50 border border-slate-200 rounded-full p-1.5">
            {([
              ...
              <button
                ...
                className={`relative font-sans text-sm font-bold py-2 px-5 rounded-full transition-colors cursor-pointer ${
                  selectedCategory === opt.id ? 'text-white' : 'text-slate-600 hover:text-slate-900'
                }`}
              >
                {selectedCategory === opt.id && (
                  <motion.span
                    layoutId="cat-pill"
                    className="absolute inset-0 bg-slate-900 rounded-full shadow-lg shadow-slate-900/15"
                    style={{ boxShadow: '0 8px 24px -10px rgba(225,29,72,0.5)' }}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredProducts.map((prod, idx) => {
            ...
            return (
              ...
              <TiltCard
                className="group bg-white rounded-3xl border-2 border-slate-150 shadow-sm hover:shadow-2xl hover:border-rose-300 hover:shadow-rose-100/50 transition-shadow duration-300 flex flex-col justify-between overflow-hidden h-full"
              >
                ...
                <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-[10px] font-black tracking-widest text-[#14B8A6] uppercase bg-teal-50 py-0.5 px-2.5 rounded-full">
                  ...
                  <div className="space-y-1 cursor-pointer group" onClick={() => onSelectProduct(prod)} title={currentLang === 'en' ? 'Click to view full details page' : '点击查看专属详情页'}>
                    <h3 className="font-sans text-lg font-extrabold text-slate-900 leading-tight group-hover:text-rose-650 transition-colors flex items-center gap-1.5 flex-wrap">
                      <span>{productName(prod)}</span>
                      <span className="text-[10px] bg-rose-50 text-rose-650 tracking-wider py-0.5 px-2 rounded font-bold uppercase shrink-0">{t.quickView}</span>
                    </h3>
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-2.5 border-t border-slate-100/70">
                    
                    {/* Colors Selector */}
                    <div className="flex flex-col space-y-1 text-left">
                      <span className="text-[9px] font-bold text-slate-450 tracking-wider uppercase">{currentLang === 'en' ? 'Color' : '选择颜色'}</span>
                      <div className="flex items-center space-x-1.5 mt-1">
                        {prod.colors.map((c) => (
                          <button
                            ...
                            className={`w-5 h-5 rounded-full border cursor-pointer focus:outline-none transition-transform ${config.colorHex === c.hex ? 'scale-125 border-rose-500 ring-2 ring-rose-100' : 'border-slate-200 hover:scale-110'}`}
                          />
                        ))}
                      </div>
                    </div>
                    ...
                    {prod.sizes && prod.sizes.length > 0 ? (
                      <div className="flex flex-col space-y-1 text-left">
                        <span className="text-[9px] font-bold text-slate-450 tracking-wider uppercase">{t.sizeLabel}</span>
                        <select
                          value={config.size}
                          onChange={(e) => updateProductConfig(prod.id, { size: e.target.value })}
                          className="mt-1 text-left text-xs font-bold text-slate-700 bg-slate-50 border border-slate-200 rounded-md py-1 px-1.5 focus:outline-none focus:ring-1 focus:ring-rose-450 cursor-pointer"
                        >
                          {prod.sizes.map((s) => (
                      ...
                      <div className="flex flex-col space-y-1 text-left">
                        <span className="text-[9px] font-bold text-slate-450 tracking-wider uppercase">{t.sizeLabel}</span>
                        <span className="text-xs font-bold text-slate-500 mt-2 block">{currentLang === 'en' ? 'One Size fits all' : '均码 / 适体配对'}</span>
                      </div>
                    )}
                  ...
                  <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                    {showPrices ? (
                      <div>
                        <span className="text-[9px] font-bold text-slate-450 tracking-wider block uppercase">{currentLang === 'en' ? 'DTC Outright Price' : '独立站直销渠道价'}</span>
                    ...
                    <div className="flex items-center space-x-2">
                      <button
                        ...
                        className="flex items-center gap-1.5 p-2.5 bg-[#F4FBF9] text-[#14B8A6] border border-teal-150 rounded-xl hover:bg-teal-50 hover:text-teal-700 transition-colors cursor-pointer"
                      ...
                      {shoppingEnabled && (
                        <button
                          ...
                          className={`flex items-center space-x-1 px-4 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${successFeedbackId === prod.id ? 'bg-emerald-500 text-white shadow-sm font-bold' : 'bg-rose-600 hover:bg-rose-700 text-white shadow-md shadow-rose-100'}`}
                        >
      ...
      {selectedProductForModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          {/* Modal Container */}
          <div className="relative bg-white rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl border-2 border-slate-150 p-6 sm:p-8 shrink-0 scrollbar-thin">
            
            {/* Close */}
            <button
              onClick={() => setSelectedProductForModal(null)}
              className="absolute top-4 right-4 p-1.5 hover:bg-slate-100 rounded-full text-slate-500 hover:text-slate-950 transition-colors cursor-pointer"
            >
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center space-x-3.5 pb-4 border-b border-slate-100">
                <div className="p-2.5 bg-rose-50 text-rose-600 rounded-2xl">
                  <Award className="w-6 h-6 fill-rose-200" />
                </div>
                <div>
                  <span className="text-[10px] font-black tracking-widest text-[#14B8A6] uppercase block">
              ...
              <div className="space-y-2">
                <p className="font-sans text-sm font-bold text-rose-650">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                ...
                <div className="bg-[#FAFAFB] p-4 rounded-2xl border-2 border-slate-150">
                  <span className="text-[10px] font-black tracking-wider text-rose-600 uppercase block mb-3">★ CORE TARGET INDICATIONS / 调理针对症状</span>
                  <div className="space-y-2">
                ...
                <div className="bg-sky-50/20 p-4 rounded-2xl border-2 border-sky-150 flex flex-col justify-between">
                  <div className="space-y-2.5">
                    <span className="text-[10px] font-bold tracking-wider text-sky-750 uppercase block">💬 HOW TO USE / 穿戴与调养方法</span>
                  ...
                  <div className="border-t border-sky-200/50 pt-3 mt-3 flex items-center space-x-2 text-[10px] font-bold text-[#14B8A6]">
                    <BadgeCheck className="w-4.5 h-4.5 text-teal-600" />
              ...
              <div className="p-4 bg-slate-900 text-white rounded-2xl border border-slate-800 space-y-3 shadow-lg">
                <div className="flex justify-between items-center pb-2 border-b border-slate-800">
                  <span className="text-[9px] font-mono tracking-widest text-[#14B8A6] font-bold">1-HOUR TEST METRIC / 内源提升对比</span>
                  <span className="text-[9px] font-mono font-bold bg-slate-800 px-2 py-0.5 rounded-full">LAB CODE: JL-2023-B</span>
                </div>
                <div className="grid grid-cols-2 gap-4 text-center">
                  <div className="p-2 bg-slate-800/50 rounded-xl">
                    <span className="text-[9px] text-zinc-400 block tracking-tight">BEFORE WEAR / 未使用前</span>
                    <span className="font-mono text-xl font-black text-zinc-300">5.2 µmol/L</span>
                  </div>
                  <div className="p-2 bg-rose-500/10 rounded-xl border border-rose-500/20">
                    <span className="text-[9px] text-rose-450 block tracking-tight">AFTER 1 HOUR WEAR / 佩戴1小时后</span>
                    <span className="font-mono text-xl font-black text-rose-500 animate-pulse">14.1 µmol/L (+271%)</span>
                  </div>
                </div>
              </div>
              ...
              {(showPrices || shoppingEnabled) && (
                <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
                  {showPrices ? (
                    <div>
                      <span className="text-[10px] font-bold text-slate-450 uppercase tracking-widest block">{currentLang === 'en' ? 'Direct Price' : '独立站到手价'}</span>
                  ...
                  {shoppingEnabled && (
                    <button
                      ...
                      className="bg-rose-600 hover:bg-rose-700 text-white font-black text-xs py-3 px-6 rounded-xl shadow-md shadow-rose-100 tracking-wider uppercase transition-all cursor-pointer"
                    >
                      {t.addToCart}
                    </button>
                  )}
      ...
      <motion.div
        ...
        style={{
          background: useTransform(
            [gx, gy] as const,
            ([x, y]) => `radial-gradient(220px circle at ${x} ${y}, rgba(244,63,94,0.18), transparent 70%)`
          )
        }}
      />
      {children}
