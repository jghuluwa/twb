...
import { X, Trash2, ShieldCheck, Truck, CreditCard, ShoppingBag, BadgeCheck, MapPin, LogIn, Lock, Check } from 'lucide-react';
  ...
  return (
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <motion.div
        ...
        className="relative w-full max-w-md bg-[#0a0f1d] text-white h-full shadow-2xl flex flex-col justify-between border-l border-white/10 z-10 p-5 sm:p-6 overflow-hidden"
      >
        {/* Panel Header */}
        <div className="flex justify-between items-center border-b border-white/10 pb-4">
          <div className="flex items-center space-x-2.5">
            <ShoppingBag className="w-5.5 h-5.5 text-cyan-300" />
            <h3 className="font-sans text-lg font-extrabold text-white">{t.cartTitle}</h3>
            <span className="text-[11px] font-mono font-black text-cyan-300 bg-cyan-500/10 py-0.5 px-2 rounded-full border border-cyan-500/30">
              {cartItems.length}
            </span>
          </div>
          <button onClick={onClose} className="p-1 hover:bg-white/10 rounded-full text-slate-400 hover:text-white transition-colors cursor-pointer">
            <X className="w-6 h-6" />
          </button>
        </div>
          ...
          {cartItems.length === 0 ? (
            ...
            <div className="space-y-4">
              {cartItems.map((item, index) => {
                const price = currency === 'USD' ? item.product.priceUSD : item.product.priceCNY;
                return (
                  <div key={index} className="flex gap-4 p-3 bg-white/[0.03] rounded-2xl border border-white/10 items-center hover:bg-white/[0.06] transition-colors">

                    {/* Size and color tiny badge visual */}
                    <div className="w-16 h-16 bg-white/5 rounded-xl flex items-center justify-center border border-white/15 overflow-hidden shrink-0 relative">
                    ...
                    <div className="flex-1 space-y-1 text-left">
                      <h4 className="font-sans text-sm font-extrabold text-white line-clamp-1 leading-tight">
                      ...
                      <div className="flex items-center justify-between pt-1">
                        <div className="flex items-center bg-white/5 border border-white/15 rounded-lg overflow-hidden p-0.5">
                          <button
                            onClick={() => onUpdateQuantity(index, -1)}
                            className="px-2 py-0.5 text-xs font-bold text-slate-400 hover:bg-white/10 hover:text-white transition-colors cursor-pointer"
                          >
                            -
                          </button>
                          <span className="px-2.5 text-xs font-mono font-bold text-slate-200">{item.quantity}</span>
                          <button
                            onClick={() => onUpdateQuantity(index, 1)}
                            className="px-2 py-0.5 text-xs font-bold text-slate-400 hover:bg-white/10 hover:text-white transition-colors cursor-pointer"
                          >
                    ...
                    <button
                      ...
                      className="p-1.5 hover:bg-rose-500/15 text-slate-400 hover:text-rose-300 rounded-lg transition-colors cursor-pointer"
                    >
        ...
        {cartItems.length > 0 && (
          <div className="border-t border-white/10 pt-4 space-y-4">
            ...
            <div className="bg-cyan-500/[0.07] text-cyan-200 p-3 rounded-xl border border-cyan-500/20 flex items-center space-x-2.5 text-xs font-bold">
              <Truck className="w-4 h-4 text-cyan-300 animate-bounce" />
            ...
            <div className="flex justify-between items-end">
              <div>
                <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block">{t.cartTotal}</span>
                <span className="text-[11px] font-bold text-slate-400">{t.paymentNotice}</span>
              </div>
            ...
            <button
              ...
              className="w-full flex items-center justify-center space-x-2 text-[#04060d] font-extrabold text-xs py-4 rounded-xl tracking-wider uppercase transition-all cursor-pointer hover:brightness-110"
      ...
      {isCheckoutModalOpen && (
        <div className="absolute inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#0a0f1d] text-white rounded-3xl w-full max-w-sm p-6 sm:p-8 shadow-2xl border border-white/10 flex flex-col justify-between overflow-hidden z-20 shrink-0 animate-in fade-in zoom-in duration-205">
            
            {/* Modal Heading */}
            <div className="flex justify-between items-center pb-3 border-b border-white/10">
              <h4 className="font-sans text-lg font-extrabold text-white flex items-center space-x-2 text-left">
            ...
            {isOrderPlaced ? (
              <div className="my-8 flex flex-col items-center justify-center text-center space-y-4 py-6">
                <div className="w-14 h-14 rounded-full bg-emerald-500/15 border border-emerald-500/30 flex items-center justify-center text-emerald-300">
                ...
                {placedOrderId && (
                  <p className="text-xs font-mono font-black text-cyan-300 bg-cyan-500/10 border border-cyan-500/30 px-3 py-1.5 rounded-lg">
                    {placedOrderId}
                  </p>
                )}
              ...
              <div className="my-6 space-y-3">
                ...
                {customer ? (
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 bg-cyan-500/10 hover:bg-cyan-500/20 border border-cyan-500/30 text-cyan-200 rounded-lg px-3 py-2 text-[11px] font-bold cursor-pointer"
                  >
                ...
                {customer && customer.addresses.length > 0 && (
                  <div className="space-y-1.5 text-left">
                    ...
                    <select
                      ...
                      className="w-full text-xs font-bold text-slate-200 bg-white/5 border border-white/15 rounded-lg p-2.5 focus:outline-none cursor-pointer"
                    >
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="space-y-1 text-left">
                    ...
                    <input type="text" required value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder={currentLang === 'en' ? 'John Doe' : '请输入姓名'} className="w-full text-xs font-bold text-slate-200 bg-white/5 border border-white/15 rounded-lg p-2.5 focus:outline-none" />
                  </div>
                  <div className="space-y-1 text-left">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">{currentLang === 'en' ? 'Email' : '邮箱 *'}</label>
                    <input type="email" required value={customerEmail} onChange={(e) => setCustomerEmail(e.target.value)} placeholder="you@email.com" className="w-full text-xs font-bold text-slate-200 bg-white/5 border border-white/15 rounded-lg p-2.5 focus:outline-none" />
                  </div>
                  <div className="space-y-1 text-left col-span-2">
                    ...
                    <input type="tel" value={customerPhone} onChange={(e) => setCustomerPhone(e.target.value)} placeholder="+86 138 0000 0000" className="w-full text-xs font-bold text-slate-200 bg-white/5 border border-white/15 rounded-lg p-2.5 focus:outline-none" />
                  </div>
                  <div className="space-y-1 text-left col-span-2">
                    ...
                    <input type="text" value={customerAddress} onChange={(e) => setCustomerAddress(e.target.value)} placeholder={currentLang === 'en' ? 'Street, City' : '街道、城市'} className="w-full text-xs font-bold text-slate-200 bg-white/5 border border-white/15 rounded-lg p-2.5 focus:outline-none" />
                  </div>
                </div>
                ...
                <div className="space-y-1.5 text-left">
                  ...
                  {discount ? (
                    <div className="flex items-center justify-between bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2 text-xs font-bold text-emerald-700">
                      <span className="inline-flex items-center gap-1.5"><Check className="w-3.5 h-3.5 shrink-0" />{discount.code} — -{currencySymbol}{discount.amount.toLocaleString()}{discount.kind === 'free_shipping' ? (currentLang === 'en' ? ' + free shipping' : ' + 免运费') : ''}</span>
                    ...
                    <div className="flex gap-2">
                      <input value={discountInput} onChange={(e) => setDiscountInput(e.target.value)}
                        ...
                        className="flex-1 text-xs font-bold text-slate-200 bg-white/5 border border-white/15 rounded-lg p-2.5 focus:outline-none" />
                      <button onClick={applyDiscount} type="button"
                        className="bg-cyan-500/15 hover:bg-cyan-500/25 border border-cyan-500/30 text-cyan-200 font-extrabold text-xs uppercase tracking-wider px-3 py-2 rounded-lg cursor-pointer">
                        {currentLang === 'en' ? 'Apply' : '应用'}
                      </button>
                    </div>
                  )}
                  {discountError && <p className="text-[11px] font-bold text-rose-300">{discountError}</p>}
                </div>
                ...
                {/* Subtotal review */}
                <div className="bg-white/[0.03] p-3.5 rounded-xl border border-white/10 space-y-1.5 text-left">
                  ...
                  <div className="flex justify-between text-sm font-extrabold text-white border-t border-white/15 pt-1.5 mt-1.5">
                ...
                {submitError && (
                  <p className="text-xs font-bold text-rose-300 bg-rose-500/10 border border-rose-500/30 px-3 py-2 rounded-lg">{submitError}</p>
                )}
                ...
                <button
                  ...
                  disabled={submitting}
                  className="w-full flex items-center justify-center gap-2 bg-white/10 hover:bg-white/20 text-slate-200 font-extrabold text-xs uppercase tracking-wider py-3 rounded-xl disabled:opacity-60 cursor-pointer"
                >
            ...
            <div className="border-t border-white/10 pt-3 text-center">
              <span className="inline-flex items-center justify-center gap-1.5 text-[9px] font-bold text-slate-400 font-sans tracking-wide">
